...
# Ronald 2015-12-27 Horrible patch to be used for the time being:
# Not my idea. See: https://github.com/oxplot/fysom/issues/1
try:
    unicode = unicode
except NameError:
...
Improved untagged responses handling suggested by Dave Baggett
    <dave@baggett.org> November 2009.
Improved thread naming, and 0 read detection contributed by Grant Edwards
    <grant.b.edwards@gmail.com> June 2010.
Improved timeout handling contributed by Ivan Vovnenko
    <ivovnenko@gmail.com> October 2010.
Timeout handling further improved by Ethan Glasser-Camp
    <glasse@cs.rpi.edu> December 2010.
Time2Internaldate() patch to match RFC2060 specification of English
    month names from bugs.python.org/issue11024 March 2011.
starttls() bug fixed with the help of Sebastian Spaeth
    <sebastian@sspaeth.de> April 2011.
Threads now set the "daemon" flag (suggested by offlineimap-project)
    April 2011.
Single quoting introduced with the help of Vladimir Marek
    <vladimir.marek@oracle.com> August 2011.
Support for specifying SSL version by Ryan Kavanagh
    <rak@debian.org> July 2013.
Fix for gmail "read 0" error provided by Jim Greenleaf
    <james.a.greenleaf@gmail.com> August 2013.
Fix for offlineimap "indexerror: string index out of range" bug provided by
    Eygene Ryabinkin <rea@codelabs.ru> August 2013."""
...
READ_POLL_TIMEOUT = 30  # Without this timeout interrupted network connections
                        # can hang reader
READ_SIZE = 32768  # Consume all available in socket

DFLT_DEBUG_BUF_LVL = 3 # Level above which the logging output goes directly
                       # to stderr
...
class Request(object):
    ...
    def __init__(
            self, parent, name=None, callback=None, cb_arg=None,
            cb_self=False):
        self.parent = parent
        self.name = name
        ...
        if not cb_self:
            ...
            self.callback_arg = (self, cb_arg)  # Self reference required in
                                                # callback arg
...
class IMAP4(object):
    ...
    Instantiate with:
        IMAP4(host=None, port=None, debug=None, debug_file=None,
            identifier=None, timeout=None, debug_buf_lvl=None)
    ...
    response_code_cre = re.compile(
        r'\[(?P<type>[A-Z-]+)( (?P<data>[^\]]*))?\]')
    # sequence_set_cre = re.compile(
    #   r"^[0-9]+(:([0-9]+|\*))?(,[0-9]+(:([0-9]+|\*))?)*$")
    untagged_response_cre = re.compile(
        r'\* (?P<type>[A-Z-]+)( (?P<data>.*))?')
    untagged_status_cre = re.compile(
        r'\* (?P<data>\d+) (?P<type>[A-Z-]+)( (?P<data2>.*))?')

    def __init__(
            self, host=None, port=None, debug=None, debug_file=None,
            identifier=None, timeout=None, debug_buf_lvl=None):
        ...
        self.tagre = re.compile(
            r'(?P<tag>' + self.tagpre + r'\d+) (?P<type>[A-Z]+) (?P<data>.*)'
        )
        if __debug__:
        ...
        if __debug__:
            if debug:
                self._mesg(
                    'connected to %s on port %s' % (self.host, self.port)
                )
        # Threading
        ...
        try:
            self.welcome = self._request_push(
                tag='continuation'
            ).get_response('IMAP4 protocol error: %s')[1]
            ...
            if self._get_untagged_response('PREAUTH'):
                ...
                raise self.error(
                    'unrecognised server welcome message: %s' %
                    'self.welcome'
                )
        ...
        try:
            ...
            if self.ssl_version == "tls1":
                ...
                raise socket.sslerror(
                    "Invalid SSL version requested: %s", self.ssl_version)
            ...
            # No ssl module, and socket.ssl has no fileno(), and does not
            # allow certificate verification
            raise socket.sslerror(
                "imaplib2 SSL mode does not work without ssl module")
    ...
    def getannotation(self, mailbox, entry, attribute, **kw):
        ...
        return self._simple_command(
            'GETANNOTATION', mailbox, entry, attribute, **kw)
    ...
    def getquotaroot(self, mailbox, **kw):
        ...
        """(typ, [[QUOTAROOT responses...],
        [QUOTA responses...]]) = getquotaroot(mailbox)
    ...
    def id(self, *kv_pairs, **kw):
        ...
        NB: a single argument is assumed to be correctly formatted and is
        ...
        if not kv_pairs:
            ...
            # Assume invoker passing correctly formatted string (back-compat)
            data = kv_pairs[0]
        else:
            data = '(%s)' % ' '.join([
                (arg and self._quote(arg) or 'NIL')
                for arg in kv_pairs
            ])
    ...
    def _CRAM_MD5_AUTH(self, challenge):
        ...
        return self.user + " " + hmac.HMAC(
            self.password, challenge).hexdigest()

    def logout(self, **kw):
        """(typ, [data]) = logout()
        Shutdown connection to server.
        Returns server 'BYE' response.
        NB: You must call this to shut down threads before discarding
            an instance."""
    ...
    def partial(self, message_num, message_part, start, length, **kw):
        ...
        return self._simple_command(
            name, message_num, message_part, start, length, **kw)
    ...
    def search(self, charset, *criteria, **kw):
        ...
        if charset:
            return self._simple_command(
                name, 'CHARSET', charset, *criteria, **kw)
    ...
    def select(self, mailbox='INBOX', readonly=False, **kw):
        ...
        so other responses should be obtained via "response('FLAGS')" etc.
        """
        self.commands_lock.acquire()
        ...
        try:
            ...
            if typ != 'OK':
                ...
                if typ == 'BAD':
                    self._deliver_exc(
                        self.error,
                        '%s command error: %s %s. Data: %.100s' %
                        (name, typ, dat, mailbox),
                        kw
                    )
                return self._deliver_dat(typ, dat, kw)
            self.state = SELECTED
        ...
        if (self._get_untagged_response('READ-ONLY', leave=True) and
                not readonly):
            if __debug__: self._dump_ur(1)
            self._deliver_exc(
                self.readonly, '%s is not writable' % mailbox, kw
            )
    ...
    def sort(self, sort_criteria, charset, *search_criteria, **kw):
        ...
        kw['untagged_response'] = name
        return self._simple_command(
            name, sort_criteria, charset, *search_criteria, **kw)

    def starttls(
            self, keyfile=None, certfile=None, ca_certs=None,
            cert_verify_cb=None, ssl_version="ssl23", **kw):
        """(typ, [data]) = starttls(
            keyfile=None, certfile=None, ca_certs=None, cert_verify_cb=None,
            ssl_version="ssl23")
        ...
        # Must now shutdown reader thread after next response, and restart
        # after changing read_fd
    ...
    def store(self, message_set, command, flags, **kw):
        ...
        kw['untagged_response'] = 'FETCH'
        return self._simple_command(
            'STORE', message_set, command, flags, **kw)
    ...
    def thread(self, threading_algorithm, charset, *search_criteria, **kw):
        """(type, [data]) = thread(
            threading_alogrithm, charset, search_criteria, ...)
        IMAPrev1 extension THREAD command."""
        name = 'THREAD'
        kw['untagged_response'] = name
        return self._simple_command(
            name, threading_algorithm, charset, *search_criteria, **kw)
        ...
        if cmdtyp == 'async':
            ...
            if need_event:
                if __debug__:
                    self._log(
                        3,
                        'sync command %s waiting for empty commands Q' % name
                    )
    ...
    def _command_completer(self, response, cb_arg, error):
        ...
        if typ == 'BAD':
            ...
            return
        if 'untagged_response' in kw:
            response = self._untagged_response(
                typ, dat, kw['untagged_response'])
        ...
        for i, (typ, dat) in enumerate(self.untagged_responses):
            if typ == name:
                ...
                if __debug__:
                    self._log(
                        5,
                        '_get_untagged_response(%s) => %s' % (name, dat)
                    )
                return dat
        self.commands_lock.release()
        return None
    ...
    def _put_response(self, resp):
        ...
        if self._literal_expected is not None:
            ...
            if self._match(self.literal_cre, dat):
                ...
                if __debug__:
                    self._log(
                        4,
                        'expecting literal size %s' %
                        self._expecting_data_len
                    )
                return
                    ...
                    if self._match(self.continuation_cre, resp):
                        if not continuation_expected:
                            if __debug__:
                                self._log(
                                    1,
                                    "unexpected continuation response: '%s'"
                                    % resp
                                )
                            return
                        self._request_pop(
                            'continuation', (True, self.mo.group('data')))
                        return
                    if __debug__:
        ...
        if (typ in ('OK', 'NO', 'BAD') and
                self._match(self.response_code_cre, dat)):
    ...
    def _request_pop(self, name, data):
        ...
        self.commands_lock.release()
        if __debug__:
            self._log(4, '_request_pop(%s, %s) = %s' % (name, data, rqb.tag))
        rqb.deliver(data)
    ...
    def _simple_command(self, name, *args, **kw):
        if 'callback' in kw:
            ...
            self._command(
                name, callback=self._command_completer, cb_arg=kw,
                cb_self=True, *args
            )
            return (None, None)
    ...
    def _untagged_response(self, typ, dat, name):
        ...
        if __debug__:
            self._log(
                4,
                '_untagged_response(%s, ?, %s) => %s' % (typ, name, data)
            )
        return typ, data
    ...
    if __debug__:
        ...
        def _init_debug(
                self, debug=None, debug_file=None, debug_buf_lvl=None):
            self.debug_lock = threading.Lock()
            self.debug = self._choose_nonull_or_dflt(0, debug, Debug)
            self.debug_file = self._choose_nonull_or_dflt(
                sys.stderr, debug_file)
            self.debug_buf_lvl = self._choose_nonull_or_dflt(
                DFLT_DEBUG_BUF_LVL, debug_buf_lvl)
            ...
            if self.debug:
                ...
                self._mesg(
                    'imaplib2 debug level %s, buffer level %s' %
                    (self.debug, self.debug_buf_lvl)
                )
...
class IMAP4_SSL(IMAP4):
    ...
    Instantiate with:
        ...
        ca_certs=None, cert_verify_cb=None, ssl_version="ssl23", debug=None,
        debug_file=None, identifier=None, timeout=None)
        ...
        ca_certs       - PEM formatted certificate chain file used to
                         validate server certificates (default: None);
        cert_verify_cb - function to verify authenticity of server
                         certificates (default: None);
        ssl_version    - SSL version to use (default: "ssl23", choose from:
                         "tls1","ssl2","ssl3","ssl23");
        debug          - debug level (default: 0 - no debug);
        debug_file     - debug stream (default: sys.stderr);
        identifier     - thread identifier prefix (default: host);
        timeout        - timeout in seconds when expecting a command response.
    ...
    def __init__(
            self, host=None, port=None, keyfile=None, certfile=None,
            ca_certs=None, cert_verify_cb=None, ssl_version="ssl23",
            debug=None, debug_file=None, identifier=None, timeout=None,
            debug_buf_lvl=None):
        self.keyfile = keyfile
        self.certfile = certfile
        self.ca_certs = ca_certs
        ...
        self.ssl_version = ssl_version
        IMAP4.__init__(
            self, host, port, debug, debug_file, identifier, timeout,
            debug_buf_lvl
        )
...
class IMAP4_stream(IMAP4):
    ...
    Instantiate with:
        IMAP4_stream(
            command, debug=None, debug_file=None, identifier=None,
            timeout=None, debug_buf_lvl=None
        )
    ...
    def __init__(
            self, command, debug=None, debug_file=None, identifier=None,
            timeout=None, debug_buf_lvl=None):
        ...
        self.read_fd = None
        IMAP4.__init__(
            self, None, None, debug, debug_file, identifier, timeout,
            debug_buf_lvl
        )
    ...
    def open(self, host=None, port=None):
        ...
        self._P = Popen(
            self.command, shell=True, stdin=PIPE, stdout=PIPE, close_fds=True)
...
class _IdleCont(object):
    ...
    def process(self, data, rqb):
        ...
        if __debug__:
            self.parent._log(
                2,
                'server IDLE started, timeout in %.2f secs' % self.timeout
            )
        return None
...
if __name__ == '__main__':
    ...
    data = open(
        os.path.exists("test.data") and "test.data" or __file__).read(1000)
    ...
    try:
        ...
        if keyfile is not None:
            ...
            M = IMAP4_SSL(
                host=host, port=port, keyfile=keyfile, certfile=certfile,
                debug=debug, identifier='', timeout=10,
                debug_buf_lvl=debug_buf_lvl
            )
        elif stream_command:
            M = IMAP4_stream(
                stream_command, debug=debug, identifier='', timeout=10,
                debug_buf_lvl=debug_buf_lvl
            )
        else:
            M = IMAP4(
                host=host, port=port, debug=debug, identifier='',
                timeout=10, debug_buf_lvl=debug_buf_lvl
            )
